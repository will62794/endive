...
EXTENDS Integers, FiniteSets, FiniteSetsExt, Randomization, TLC
...
vars == <<round, step, decision, lockedValue, lockedRound,validValue, validRound, evidence, msgsPropose, msgsPrevote, msgsPrecommit, Proposer>>
...
\* ASSUME PrintT(TLCEval(kSubset(2, AllFaultyProposals)))
...
Init ==
    ...
    /\ msgsPropose \in [Rounds -> RandomSetOfSubsets(8, 2, AllFaultyProposals)]
    /\ BenignRoundsInMessages(msgsPropose)
    \* /\ msgsPrevote \in [Rounds -> SUBSET AllFaultyPrevotes]
    /\ msgsPrevote \in [Rounds -> RandomSetOfSubsets(8, 2, AllFaultyPrevotes)]
    /\ BenignRoundsInMessages(msgsPrevote)
    \* /\ msgsPrecommit \in [Rounds -> SUBSET AllFaultyPrecommits]
    /\ msgsPrecommit \in [Rounds -> RandomSetOfSubsets(8, 2, AllFaultyPrecommits)]
    /\ BenignRoundsInMessages(msgsPropose)
    /\ BenignRoundsInMessages(msgsPrevote)
...
InsertProposal(p) == 
  ...
  /\ UNCHANGED <<evidence, round, decision, lockedValue, lockedRound,
                validValue, step, validRound, msgsPrevote, msgsPrecommit, Proposer>>
...
UponProposalInPropose(p) ==
  \E v \in Values:
    ...
    /\ UNCHANGED <<round, decision, lockedValue, lockedRound,
                   validValue, validRound, msgsPropose, msgsPrecommit, Proposer>>
...
UponQuorumOfPrevotesAny(p) ==
  ...
  /\ \E MyEvidence \in SUBSET msgsPrevote[round[p]]:
      ...
      /\ UNCHANGED <<round, decision, lockedValue, lockedRound,
                    validValue, validRound, msgsPropose, msgsPrevote, Proposer>>
  ...
  \E v \in ValidValues, vr \in RoundsOrNil:
    ...
    /\  IF step[p] = "PREVOTE"
        THEN \* lines 38-41:
          ...
          /\ step' = [step EXCEPT ![p] = "PRECOMMIT"]
        ELSE
          UNCHANGED <<lockedValue, lockedRound, msgsPrecommit, step, Proposer>>
      \* lines 42-43
    /\ validValue' = [validValue EXCEPT ![p] = v]
    /\ validRound' = [validRound EXCEPT ![p] = round[p]]
    /\ UNCHANGED <<round, decision, msgsPropose, msgsPrevote, Proposer>>
...
UponQuorumOfPrecommitsAny(p) ==
  /\ \E MyEvidence \in SUBSET msgsPrecommit[round[p]]:
      ...
      /\ UNCHANGED <<decision, lockedValue, lockedRound, validValue,
                    validRound, msgsPropose, msgsPrevote, msgsPrecommit, Proposer>>
...
OnTimeoutPropose(p) ==
  ...
  /\ step' = [step EXCEPT ![p] = "PREVOTE"]
  /\ UNCHANGED <<round, lockedValue, lockedRound, validValue,
                validRound, decision, evidence, msgsPropose, msgsPrecommit, Proposer>>
...
OnQuorumOfNilPrevotes(p) ==
  ...
  /\ LET PV == { m \in msgsPrevote[round[p]]: m.id = Id(NilValue) } IN
    ...
    /\ step' = [step EXCEPT ![p] = "PRECOMMIT"]
    /\ UNCHANGED <<round, lockedValue, lockedRound, validValue,
                  validRound, decision, msgsPropose, msgsPrevote, Proposer>>
