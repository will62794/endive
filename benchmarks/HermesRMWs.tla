...
VARIABLES   Rmsgs,
            ...
            committedRMWs,
            committedWrites
msgs == <<msgsINV, msgsVAL, msgsACK>>                         
\* all Hermes (+ environment, + RMW) variables
hrvars == << msgsINV, msgsVAL, msgsACK, nodeTS, nodeState, nodeRcvedAcks, nodeLastWriter, 
...
HRSemanticsRMW ==  \* The invariant that an we cannot have two operations committed 
    ...
    /\ \A x \in committedRMWs:
       \A y \in committedWrites: (x.version /= y.version) /\ (x.version /= y.version - 1)
...
HRRcvWriteInv(n) ==  \* Process a received invalidation for a write
    \E m \in Rmsgs: 
        ...
        /\ h_send_inv_or_ack(n, m.version, m.tieBreaker, "ACK", msgsACK) 
        ...
        /\ UNCHANGED <<nodeLastWriteTS, aliveNodes, nodeRcvedAcks, Rmsgs, 
                       epochID, nodeWriteEpochID, committedRMWs, committedWrites, msgsVAL, msgsINV>>
    ...
    \E m \in Rmsgs: 
        ...
        /\ IF greaterTS(m.version, m.tieBreaker,
           ...
           THEN
                ...
                /\ h_send_inv_or_ack(n, m.version, m.tieBreaker, "ACK", msgsACK) 
                /\ hr_upd_state_greater_inv(n)
                /\ UNCHANGED <<Rmsgs, msgsINV, msgsVAL>>
            ELSE IF equalTS(m.version, m.tieBreaker,
                            nodeTS[n].version, nodeTS[n].tieBreaker)
            THEN
                \* acknowledge a received invalidation (w/ equal timestamp)
                /\ h_send_inv_or_ack(n, m.version, m.tieBreaker, "ACK", msgsACK) 
                /\ UNCHANGED <<nodeState, nodeTS, nodeLastWriter, nodeFlagRMW, Rmsgs, msgsINV, msgsVAL>>
            ELSE \* smaller TS
