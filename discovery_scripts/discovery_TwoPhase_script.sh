#!/bin/bash
#SBATCH -J TwoPhase_endive_Job               # Job name
...
#SBATCH -o endive_logs/TwoPhase/TwoPhase_output_%j.txt       # Standard output file
#SBATCH -e endive_logs/TwoPhase/TwoPhase_error_%j.txt        # Standard error file
...
echo "---"
echo "--- Running '$specname' benchmark with seed $seed (BASELINE) ---"
echo "---"
srun python3 endive.py --spec benchmarks/$specname \
        --seed $seed --num_simulate_traces 200000 --tlc_workers 28 \
        --debug --target_sample_time_limit_ms 10000 --target_sample_states 200000 \
